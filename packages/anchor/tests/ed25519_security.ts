...
function signMessage(
  ...
  // * tweetnacl.sign returns [signature (64 bytes) + message]
  // * Extract first 64 bytes (signature) from the result
  const signed = nacl.sign(message, keypair.secretKey);
  return signed.slice(0, 64);
}

// * Helper: Convert domain string to fixed-size array [u8; 32] for Rust
function domainToArray(domainStr: string): number[] {
  const domain = new Uint8Array(32);
  const domainBytes = Buffer.from(domainStr, "utf8");
  domain.set(domainBytes.slice(0, 32), 0);
...
describe("Ed25519 Security Tests", () => {
  ...
  it("should accept valid Ed25519 signature", async () => {
    ...
    // * Derive PDA with authority seed (unique per user, nullifier validated in instruction logic)
    const [nullifierPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("nullifier"), authority.publicKey.toBuffer()],
      VEILED_PROGRAM_ID
    );
